...
public class SkyDatabase {
	...
	class DBHelper extends SQLiteOpenHelper {
	    ...
	    public void onCreate(SQLiteDatabase db) {
	        ...
	        String sql = "INSERT INTO Setting(BookCode,FontName,FontSize,LineSpacing,Foreground,Background,Theme,Brightness,TransitionType,LockRotation) VALUES(0,'',2,-1,-1,-1,0,1,2,1)";
	        db.execSQL(sql);
	    }
    ...
    */
    public PagingInformation fetchPagingInformation(PagingInformation pgi) {
    	String sql = String.format(Locale.US,"SELECT * FROM Paging WHERE BookCode=%d AND ChapterIndex=%d AND FontName='%s' AND FontSize=%d AND LineSpacing=%d AND Width=%d AND Height=%d AND HorizontalGapRatio=%f AND VerticalGapRatio=%f AND IsPortrait=%d AND IsDoublePagedForLandscape=%d",
    ...
    public void updatePosition(int bookCode,double position) {
    	...
    	values.put("IsRead", 1);
    	String where = String.format("BookCode=%d",bookCode);
 ...
 // Using db method
    public void updateBook(BookInformation bi) {
    	...
    	String where = String.format("BookCode=%d"
    			,bi.bookCode
    	);
    	db.update("Book", values, where, null);
    }
...
//    } 
    ...
    public ArrayList<BookInformation> fetchBookInformations(int sortType,String key) {
    	...
    	if (!(key==null || key.isEmpty())) {
    		condition =String.format(" WHERE Title like '%%%s%%' OR Author like '%%%s%%'",key,key);
    	}
    ...
    public String getFileNameByBookCode(int bookCode) {
    	String numberPart = String.format("%07d",bookCode);
