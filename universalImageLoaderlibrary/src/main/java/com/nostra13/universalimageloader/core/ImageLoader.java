...
public class ImageLoader {
	...
	public void displayImage(String uri, ImageView imageView, ProgressBar progressBar) {
		displayImage(uri, imageView, null, null, progressBar);
	}
	 ...
	 * @throws IllegalArgumentException if passed <b>imageView</b> is null
	 */
	public void displayImage(String uri, ImageView imageView, ImageLoadingListener listener, ProgressBar progressBar) {
		displayImage(uri, imageView, null, listener, progressBar);
	}
	 ...
	 * @throws IllegalArgumentException if passed <b>imageView</b> is null
	 */
	public void displayImage(String uri, ImageView imageView, DisplayImageOptions options, ImageLoadingListener listener, ProgressBar progressBar) {
		checkConfiguration();
		if (imageView == null) {
		...
		if (bmp != null && !bmp.isRecycled()) {
			...
			if (options.shouldPostProcess()) {
				...
				ProcessAndDisplayImageTask displayTask = new ProcessAndDisplayImageTask(engine, bmp, imageLoadingInfo, options.getHandler(), progressBar);
				engine.submit(displayTask);
			} else {
//				options.getDisplayer().display(bmp, imageView, LoadedFrom.MEMORY_CACHE);
				options.getDisplayer().display(bmp, imageView, LoadedFrom.MEMORY_CACHE, progressBar);
...
//				imageView.setImageResource(options.getStubImage());
			...
			ImageLoadingInfo imageLoadingInfo = new ImageLoadingInfo(uri, imageView, targetSize, memoryCacheKey, options, listener, engine.getLockForUri(uri));
			LoadAndDisplayImageTask displayTask = new LoadAndDisplayImageTask(engine, imageLoadingInfo, options.getHandler(), progressBar);
			engine.submit(displayTask);
		}
	}
	...
	public void loadImage(String uri, ImageSize targetImageSize, DisplayImageOptions options, ImageLoadingListener listener) {
		...
		displayImage(uri, fakeImage, optionsWithFakeDisplayer, listener, null);//changed remove last argument null
	}
